...
names(timing_data_short)
	...
	tds_by_event = tds_by_event[grep('gazetracking-test-normal', tds_by_event)]

	frame_events = do.call('rbind', lapply(tds_by_event, function(x){
		if (!('exp-lookit-video:videoStarted' %in% x$eventType)){
			return(NULL)			
		}		
		start_stimulus_video_time =  min(subset(x, eventType == "exp-lookit-video:videoStarted")$timestamp)		
		recorder_start_time = subset(x, eventType == "exp-lookit-video:startRecording")$timestamp		
		recorder_end_time = max(subset(x, eventType == "exp-lookit-video:stoppingCapture")$timestamp)
		
		end_stimulus_video_time =  max(subset(x, eventType == "exp-lookit-video:videoStopped")$timestamp)
		
		return(data.frame(start_stimulus_video_time, end_stimulus_video_time, recorder_start_time, video_stream_connection_time, video_stream_disconnection_time,
	...
	if (browser %in% c('chrome', 'firefox')){
		# recorder_start_time is the reference
		time_cols = c('start_stimulus_video_time', 'end_stimulus_video_time', 'video_stream_connection_time', 'video_stream_disconnection_time', 'recorder_end_time')
...
get_timecourse_info_for_videos = function(base_video_path, frame_data_path, name_type, browser, gaze_code_path=NULL){
	...
	if (length(grep('_trimmed', video_paths)) > 0){
	...
	target_cols = c('frame_id','recorder_start_time', 'recorder_end_time', 'mediainfo_video_duration', 'mediainfo_audio_duration', 'trial_type', 'trial_index', 'start_stimulus_video_time', 'end_stimulus_video_time')
	if (!is.null(gaze_code_path)){		
		...
		target_cols = c(target_cols, 'earliest_nonfrozen_gaze', 'latest_nonfrozen_gaze', 'earliest_gaze', 'latest_gaze')
...
plot_normalized_frame_events = function(normalized_frame_events, browser){ 
	...
	} else if (browser == 'firefox_nogaze'){
		p1 = p1 + xlab('Seconds (Firefox: 0 = startRecoding)')
	} else if (browser == 'chrome'){
		p1 = p1 + xlab('Seconds (Chrome: 0 = startRecording = earliest_gaze = earliest_nonfrozen_gaze)')	
	} else if (browser == 'chrome_nogaze'){
		p1 = p1 + xlab('Seconds (Chrome: 0 = startRecording)')	
	}
	return(p1)
 }
